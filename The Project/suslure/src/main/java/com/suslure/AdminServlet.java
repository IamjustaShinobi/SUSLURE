...
/**
 * AdminServlet — SUSLURE Admin Dashboard.
 * Protected by username: SusLure / password: thats a secret baby
 * Shows all attack logs with network scanner enrichment data.
 * GET  /admin      → shows login or dashboard
 * POST /admin/auth → authenticates admin
 * POST /admin      → clear/simulate actions
 */
public class AdminServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        res.getWriter().write(buildDashboard(records, total, sqli, brute, hijack, normal, uniq, filter));
    }
...
"<title>SUSLURE — Admin Access</title>\n" +
"<style>\n" +
"@import url('https://fonts.googleapis.com/css2?family=Share+Tech+Mono&family=Orbitron:wght@700;900&family=Rajdhani:wght@500;600&display=swap');\n" +
"*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}\n" +
"body{background:#000;font-family:'Rajdhani',sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;position:relative;overflow:hidden}\n" +
...
".grid{position:fixed;inset:0;background-image:linear-gradient(rgba(0,255,70,0.035) 1px,transparent 1px),linear-gradient(90deg,rgba(0,255,70,0.035) 1px,transparent 1px);background-size:44px 44px;pointer-events:none}\n" +
".glow{position:fixed;width:600px;height:600px;background:radial-gradient(circle,rgba(0,255,70,0.06),transparent 60%);top:50%;left:50%;transform:translate(-50%,-50%);pointer-events:none}\n" +
".card{width:100%;max-width:400px;background:rgba(0,8,2,0.88);border:1px solid rgba(0,255,70,0.25);padding:44px 40px;position:relative;z-index:1;animation:ci .6s ease;box-shadow:0 0 60px rgba(0,255,70,0.06)}\n" +
...
".corner{position:absolute;width:10px;height:10px;border-color:rgba(0,255,70,0.4);border-style:solid}\n" +
...
"h1{font-family:'Orbitron',sans-serif;font-size:20px;font-weight:900;color:#00ff46;letter-spacing:4px;text-align:center;margin-bottom:4px;text-shadow:0 0 20px rgba(0,255,70,0.4)}\n" +
".sub{font-family:'Share Tech Mono',monospace;font-size:9px;color:rgba(0,255,70,0.35);letter-spacing:3px;text-align:center;margin-bottom:32px}\n" +
".lbl{display:block;font-family:'Share Tech Mono',monospace;font-size:9px;color:rgba(0,255,70,0.5);letter-spacing:3px;margin-bottom:7px;text-transform:uppercase}\n" +
".fg{margin-bottom:16px}\n" +
".fw{background:rgba(0,255,70,0.04);border:1px solid rgba(0,255,70,0.18);display:flex;align-items:center;transition:border-color .2s,box-shadow .2s}\n" +
".fw:focus-within{border-color:rgba(0,255,70,0.6);box-shadow:0 0 0 3px rgba(0,255,70,0.1)}\n" +
".fw input{width:100%;background:transparent;border:none;outline:none;color:#00ff46;font-family:'Share Tech Mono',monospace;font-size:13px;padding:13px 16px;letter-spacing:1px}\n" +
".fw input::placeholder{color:rgba(0,255,70,0.2)}\n" +
".btn{width:100%;background:transparent;border:1px solid rgba(0,255,70,0.45);color:#00ff46;font-family:'Orbitron',sans-serif;font-size:11px;font-weight:700;padding:14px;cursor:pointer;letter-spacing:3px;text-transform:uppercase;transition:all .25s;margin-top:8px}\n" +
".btn:hover{border-color:#00ff46;background:rgba(0,255,70,0.08);box-shadow:0 0 25px rgba(0,255,70,0.2)}\n" +
".msg-err{background:rgba(255,40,40,0.08);border:1px solid rgba(255,40,40,0.4);border-left:2px solid #ff3b3b;color:#ff6b6b;font-family:'Share Tech Mono',monospace;font-size:11px;padding:10px 14px;margin-bottom:16px;letter-spacing:.5px}\n" +
".back{display:block;text-align:center;font-family:'Share Tech Mono',monospace;font-size:9px;color:rgba(0,255,70,0.25);letter-spacing:2px;text-decoration:none;margin-top:20px;transition:color .2s}\n" +
...
"  <h1>SUSLURE</h1>\n" +
"  <p class='sub'>ADMIN ACCESS · AUTHORIZED ONLY</p>\n" +
errHtml +
"  <form method='POST' action='/admin'>\n" +
"    <input type='hidden' name='action' value='auth'/>\n" +
"    <div class='fg'><label class='lbl'>Admin Username</label>\n" +
"      <div class='fw'><input type='text' name='username' placeholder='Enter username...' required/></div></div>\n" +
"    <div class='fg'><label class='lbl'>Admin Password</label>\n" +
"      <div class='fw'><input type='password' name='password' placeholder='Enter password...' required/></div></div>\n" +
"    <button type='submit' class='btn'>[ ACCESS DASHBOARD ]</button>\n" +
"  </form>\n" +
"  <a href='/login' class='back'>← RETURN TO PORTAL</a>\n" +
"</div></body></html>";
    }
    ...
    private String buildDashboard(List<IntrusionRecord> records,
        ...
        if (records.isEmpty()) {
            rows.append("<tr><td colspan='10' class='empty'>◈ No intrusion records found. Waiting for attackers...</td></tr>");
        } else {
            for (IntrusionRecord r : records) {
                ...
                rows.append("<tr class='").append(r.getCssClass()).append("'>")
                    ...
                    .append("<td class='payload' style='color:").append(ac).append("'>").append(esc(trunc(r.getPayloadUsed(), 30))).append("</td>")
                    .append("<td class='mono dim ts'>").append(esc(r.getTimestamp())).append("</td>")
                    .append("<td class='mono green2'>").append(esc(trunc(r.getMacAddress(), 17))).append("</td>")
                    .append("<td class='dim'>").append(esc(trunc(r.getVendor(), 12))).append("</td>")
                    .append("<td class='dim ua'>").append(esc(trunc(r.getUserAgent(), 30))).append("</td>")
                    .append("</tr>");
            }
        }
...
"<title>SUSLURE — Attack Dashboard</title>\n" +
"<style>\n" +
...
".sc{background:rgba(0,5,1,0.8);border:1px solid rgba(0,255,70,0.12);padding:18px 16px;transition:border-color .2s,box-shadow .2s;position:relative;overflow:hidden}\n" +
...
".sn{font-family:'Orbitron',sans-serif;font-size:30px;font-weight:900;display:block;line-height:1;margin-bottom:6px}\n" +
".sl{font-family:'Share Tech Mono',monospace;font-size:8px;letter-spacing:2px;opacity:.5;text-transform:uppercase;display:block}\n" +
".sc-t .sn{color:#00ff46;text-shadow:0 0 20px rgba(0,255,70,0.4)}\n" +
".sc-s .sn{color:#ff3b3b}.sc-b .sn{color:#ff8c00}.sc-h .sn{color:#b026ff}\n" +
".sc-n .sn{color:#00ff88}.sc-i .sn{color:#facc15}\n" +
".controls{display:flex;align-items:center;gap:8px;margin-bottom:18px;flex-wrap:wrap}\n" +
".fb{font-family:'Share Tech Mono',monospace;font-size:9px;letter-spacing:2px;padding:7px 12px;border:1px solid rgba(0,255,70,0.15);background:transparent;color:rgba(0,255,70,0.4);cursor:pointer;transition:all .2s;text-decoration:none;display:inline-block}\n" +
".fb:hover,.fb.act{color:#00ff46;border-color:rgba(0,255,70,0.5);background:rgba(0,255,70,0.06);box-shadow:0 0 12px rgba(0,255,70,0.08)}\n" +
...
".sp{flex:1}\n" +
".ab{font-family:'Share Tech Mono',monospace;font-size:9px;letter-spacing:1px;padding:7px 12px;border:1px solid;background:transparent;cursor:pointer;transition:all .2s;text-decoration:none;display:inline-block}\n" +
".ab-sim{color:rgba(0,255,70,0.7);border-color:rgba(0,255,70,0.25)}\n" +
".ab-sim:hover{color:#00ff46;border-color:rgba(0,255,70,0.6);box-shadow:0 0 15px rgba(0,255,70,0.12)}\n" +
".ab-clr{color:rgba(255,59,59,0.7);border-color:rgba(255,59,59,0.25)}\n" +
".ab-clr:hover{color:#ff3b3b;border-color:rgba(255,59,59,0.6);box-shadow:0 0 15px rgba(255,59,59,0.1)}\n" +
...
"thead th{background:rgba(0,4,1,0.95);color:rgba(0,255,70,0.6);font-family:'Share Tech Mono',monospace;font-size:8px;letter-spacing:2px;text-transform:uppercase;padding:12px 12px;text-align:left;border-bottom:1px solid rgba(0,255,70,0.12);white-space:nowrap}\n" +
"tbody td{padding:10px 12px;border-bottom:1px solid rgba(0,255,70,0.05);vertical-align:middle;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}\n" +
...
".badge{font-family:'Share Tech Mono',monospace;font-size:9px;padding:3px 8px;border:1px solid;white-space:nowrap;letter-spacing:1px}\n" +
...
".ua{font-size:11px;max-width:160px}\n" +
".ts{font-size:10px;color:rgba(0,255,70,0.3)}\n" +
".empty{text-align:center;padding:50px;color:rgba(0,255,70,0.2);font-family:'Share Tech Mono',monospace;font-size:12px;letter-spacing:2px}\n" +
".rn{font-family:'Share Tech Mono',monospace;font-size:9px;color:rgba(0,255,70,0.2);text-align:right;margin-top:8px;letter-spacing:1px}\n" +
...
"</style></head><body>\n" +
"<div class='topbar'>\n" +
"  <div><span class='brand'>◈ SUSLURE</span><span class='brand-sub'>ATTACK INTELLIGENCE DASHBOARD</span></div>\n" +
...
"    <div class='sc sc-n'><span class='sn'>" + normal + "</span><span class='sl'>🔐 Normal Attempts</span></div>\n" +
...
"  <div class='tw'><table>\n" +
"    <thead><tr>" +
"      <th>#</th><th>TYPE</th><th>IP ADDRESS</th><th>USERNAME</th><th>PASSWORD</th>" +
"      <th>PAYLOAD</th><th>TIMESTAMP</th><th>MAC</th><th>VENDOR</th><th>USER AGENT</th>" +
"    </tr></thead>\n" +
"    <tbody>" + rows + "</tbody>\n" +
"  </table></div>\n" +
"  <div class='rn'>AUTO-REFRESH 15s · " + records.size() + " RECORDS · SUSLURE v1.0</div>\n" +
"</div>\n" +
"<footer>SUSLURE HONEYPOT · DETECT · TRAP · EXPOSE · DESTROY</footer>\n" +
"</body></html>";
    }
